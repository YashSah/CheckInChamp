...
import 'package:check_in_champ/models/user_model.dart';
...
class DbService extends ChangeNotifier{
  ...
  String generateRandomEmployeeId(){
    final random = Random();
    const allChars = 'checkinchampCHECKINCHAMP123456789';
  ...
  Future<UserModel> getUserData() async{
    final userData = await _supabase.from(
        Constants.employeeTable).select().eq('id', _supabase.auth.currentUser!.id).single();
